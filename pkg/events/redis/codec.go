...
func encodeEventMeta(evt events.Event, id *ttnpb.EntityIdentifiers) []string {
	meta := []string{
	...
	if ePB, err := events.Proto(evt); err == nil {
		...
		if rb, err := proto.Marshal(sparsePB); err == nil {
			meta = append(meta, eventSparseKey, base64.StdEncoding.EncodeToString(rb))
		}
	}
	return meta
}
